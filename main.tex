...
\node [mybox] (box){
    ...
    \setlength\itemsep{0em}
        ...
        [\sum\limits_{i=1}^{m} a_i^2 \cdot f_i] - \overline{x}^2 = \overline{x^2} - \overline{x}^2$
        \item $s_x = \sqrt{s_x^2}$
...
\node [mybox] (box){
    ...
    \begin{tabular}{lp{6.5cm} l}
        $\Omega$: & Ergebnisraum. Alle möglichen ergebnisse eines Zufallsexperiments.\\
        $\rho$: & Zähldichte (PMF). $\rho$ ist eine Funktion die angibt mit welcher Wahrscheinlichkeit die möglichen Ergebnisse des Zufallsexperiments auftreten.\\
        $E(X)$: & Erwartungswert $(=\mu)$\\
        $V(X)$: & Varianz $(=\sigma^2)$\\
        $S(X)$: & Standardabweichung $(=\sigma)$
	\end{tabular}
    \end{minipage}
};
\node[fancytitle, right=10pt] at (box.north west) {Grundbegriffe};
...
\node [mybox] (box){
    \begin{minipage}{0.295\textwidth}
        ...
        \begin{itemize}
            \setlength\itemsep{0em}
                \item Schreibweise: $X \sim Ber(p)$
                \item $P(X = 1) = p$ und $P(X = 0) = 1 - p = q$
                \item $\mu = E(X) = p$
                \item $\sigma^2 = V(X) = p \cdot q = p \cdot (1 - p)$
                \item $\sigma = S(X) = \sqrt{p \cdot q} = \sqrt{p \cdot (1 - p)}$
            \end{itemize}
    \end{minipage}
};
\node[fancytitle, right=10pt] at (box.north west) {Bernoulli Verteilung};
\end{tikzpicture}
...
\begin{tikzpicture}
    \node [mybox] (box){
        \begin{minipage}{0.295\textwidth}
            \includegraphics[width=\textwidth]{images/bsp_intervall.png}
        \end{minipage}
    };
    \node[fancytitle, right=10pt] at (box.north west) {Bsp. Intervalwarscheinlichkeit};
    \end{tikzpicture}
